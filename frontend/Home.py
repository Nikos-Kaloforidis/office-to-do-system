    ...
    with st.container(border=True):
        ...
        with col_status:
            if st.session_state.get("editing_task") == task_id:
                ...
                default_idx = (
                    options.index(current_status) if current_status in options else 0
                )
        ...
        with f2:
            if st.session_state.get("editing_task") != task_id:
                if st.button(
                    "Edit", key=f"{prefix}_ed_{task_id}", use_container_width=True
                ):
...
def render_app_header(user_data, dept_data):
    with st.container():
        ...
        if c4.button(
            "Logout", type="primary", use_container_width=True, key="main_logout"
        ):
...
if not st.session_state.get("logged_in"):
    st.markdown(
        "<h2 style='text-align: center;'>Member Login</h2>", unsafe_allow_html=True
    )
        ...
        if st.form_submit_button("Create Task", type="primary"):
            if t_name:
                payload = {
                    ...
                    "created_by_id": uid,
                    "assigned_user_id": (
                        u_opts.get(sel_user) if sel_user != "None" else None
                    ),
                    "assigned_dep_id": (
                        d_opts.get(sel_dept) if sel_dept != "None" else None
                    ),
