    ...
    if st.session_state.get("username") == "admin":
        ...
        if st.session_state.show_add_form:
            with st.form("add_dept_form", clear_on_submit=True):
                ...
                domain = st.selectbox(
                    "Domain", options=["HR", "Finance", "Support", "Developers"]
                )
        ...
        if st.session_state.show_del_form:
            with st.form("Delete Department", clear_on_submit=True):
                department_name = st.selectbox(
                    "Department", [dep["name"] for dep in dept_list]
                )
                            ...
                            with col_action:
                                with st.popover("Manage"):
                                    ...
                                    new_pwd = st.text_input(
                                        "New Password",
                                        type="password",
                                        key=f"pwd_{user['user_id']}",
                                    )
                                    if st.button(
                                        "Update Password",
                                        key=f"upd_{user['user_id']}",
                                        use_container_width=True,
                                    ):
                                        if new_pwd:
                                            try:
                                                client.patch(
                                                    f"api/users/update/password/{user['user_id']}",
                                                    {"password": new_pwd},
                                                )
                                    ...
                                    if st.button(
                                        "Delete User",
                                        key=f"del_{user['user_id']}",
                                        type="primary",
                                        use_container_width=True,
                                    ):
                                        try:
                                            client.delete(
                                                f"api/users/remove/{user['user_id']}"
                                            )
                                            st.toast(
        ...
        t_all, t_req, t_work, t_done = st.tabs(
            ["All", "Requested", "Working", "Completed"]
        )
