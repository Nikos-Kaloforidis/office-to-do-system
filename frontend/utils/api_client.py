...
class APIClient:
    ...
    def post(self, endpoint: str,  data:Dict) -> Dict[str, Any]:
        response = requests.post(f"{API_BASE}/{endpoint}", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
    
    def patch(self, endpoint: str,  data:Dict) -> Dict[str, Any]:
        response = requests.patch(f"{API_BASE}/{endpoint}", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
    ...
    def put(self, endpoint: str,  data: Dict[str, Any]) -> Dict[str, Any]:
        response = requests.put(f"{API_BASE}/{endpoint}", json=data, headers=self.headers)
        response.raise_for_status()
        return response.json()
